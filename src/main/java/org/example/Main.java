...
import org.example.data_object.*;
...
public class Main {
    ...
    public static void main(String[] args) {
       ...
       WarehouseManager warehouseManager = new WarehouseManager();
        ...
        while (running) {
           ...
           handleMenuSeletion(selection, warehouseManager, productManager);
        }
    ...
    private static void addInitialData(WarehouseManager warehouseManager, ProductManager productManager) {
        ...
        Product usbCharger = new Product("Usb C charger", 5050, "Some darn phone charger");

        productManager.addNewObject(iphone);
        productManager.addNewObject(usbCharger);

        warehouseManager.addProductToWarehouse(kista.getId(), iphone);
        warehouseManager.addProductToWarehouse(kista.getId(), usbCharger);
        warehouseManager.addProductToWarehouse(kista.getId(), iphone);

        warehouseManager.addProductToWarehouse(gothenburg.getId(), usbCharger);
    }
    ...
    private static void handleMenuSeletion(int selection, WarehouseManager warehouseManager, ProductManager productManager) {
        switch(selection) {
            case 1:
    ...
    private static void removeProductFromWarehouse(WarehouseManager warehouseManager, ProductManager productManager) {
        ...
        int productId = getNumberInput("Enter warehouse ID: ");

        try{
        warehouseManager.removeProductFromWarehouse(productId, warehouseId);
    }catch (IndexOutOfBoundsException e) {
            System.out.println("Invalid product ID or warehouse ID");
        }
    ...
    private static void addProductToWarehouse(WarehouseManager warehouseManager, ProductManager productManager) {
        ...
        Product product = productManager.getObjetById(productId);
